...
def main(model_path, input_file, output_file, image_path):
    ...
    for line in tqdm(questions):
        ...
        template_mapping = {
            "WTQ_for_TQA": wtq_image_template,
            "TABMWP_for_TQA": tabmwp_image_template,
            "TAT-QA_for_TQA": tat_qa_image_template,
            "HiTab_for_TQA": hitab_image_template,
            "FeTaQA_for_TQA": fetaqa_image_template,
            "InfoTabs_for_TFV": infotabs_image_template,
            "TabFact_for_TFV": tabfact_image_template
        }

        template = next((template_mapping[key] for key in template_mapping if key in category), None)

        if "TFV" in category:
            cur_prompt = f"{template}Statement: {question}"
        else:
            cur_prompt = f"{template}Question: {question}"
